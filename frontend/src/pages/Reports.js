...
const Reports = () => {
  ...
  const fetchProductStatusData = async () => {
    try {
      const response = await reportsAPI.getProductStatusData(selectedStatusProduct);
      console.log('Product Status Response:', response.data);
      setStatusData(response.data.chartData || []);
      setStatusSummary(response.data.summary || null);
    } catch (error) {
